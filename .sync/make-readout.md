...
1. Collect these prerequisites from the user, using names and links only, never credential
values:
...
1. Walk through each placeholder one at a time, restate the mapping back to the user for
confirmation, and have the user save the filled values into their own copy of this skill.
1. Until setup is complete, the skill cannot resolve an account, reconcile evidence, or file
a readout. It can only explain the inputs and output it would use.
1. When every placeholder is filled and confirmed, set `setup: complete` in the frontmatter
